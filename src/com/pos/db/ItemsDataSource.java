...
public class ItemsDataSource {
	...
	private final static String TAG = ItemsDataSource.class.getSimpleName();

	public ItemsDataSource(Context c, Activity ac) {
	...
	public void UpdateItem(String name, String nameCN, String itemid,
			...
			String BaseStock, String idGr, String ts) {
	...
	public boolean checkItemCode(String itemCode) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "select Item_code from items where Item_code = '"
						+ itemCode + "'";
	...
	public int updateActice(String isAc, String ID) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				ContentValues cv = new ContentValues();
				cv.put("Status", isAc);
				String whereClause = "Item_code" + "=?";
				String[] whereArgs = new String[] { ID };

				numRowEffect = dbHelper.getDb().update("items", cv,
	...
	public String checkTotalamount(String maVatTu) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "select Total_amount from sales where SaleID = '"
						+ maVatTu + "'";
				c = dbHelper.getDb().rawQuery(query, null);
	...
	public String loadPaymentNettRe1(String SaleID) {
		String s = "";
		Cursor c = null;
		int cash = 0;
		try {
	...
	public String loadPaymentRe1(String SaleID) {
		String s = "";
		Cursor c = null;
	...
	private void updateQuality(String id, String newquality) {
		List<NameValuePair> params = new ArrayList<NameValuePair>();
		params.add(new BasicNameValuePair("tag", "updateQuality"));
		params.add(new BasicNameValuePair("ItemID", id));
		params.add(new BasicNameValuePair("Quality", newquality));
	}
	...
	public ArrayList<ItemsModel> loadItemsBykey() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				...
				if (c != null && c.getCount() > 0) {
					...
					int itemTs = c.getColumnIndex("TextSize");
					...
					do {
						try {
							...
							vt.setUom(uom);
							vt.setTextSize(c.getString(itemTs));
							result.add(vt);
						} catch (Exception e) {
						}
					} while (c.moveToNext());
	...
	public String getEnglishIfDontHaveChinese(String grCode) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "SELECT item_translations.Name FROM items INNER JOIN item_translations ON items .ItemID=item_translations.ItemID WHERE item_translations.LanguageID=1"
						+ " AND Item_group_ID="
						+ Utilities.getGlobalVariable(context).posGroup
						+ " AND Item_code='" + grCode+"'";
				c = dbHelper.getDb().rawQuery(query, null);
					...
					if (c != null && c.getCount() > 0) {
						...
						do {
							try {
								...
								vt.setItemID(maVattu);
								if(name.equals("")){
									vt.setName(getEnglishIfDontHaveChinese(itemcode));
								}else{
									vt.setName(name);
								}
	...
	public String getTs(String maVatTu) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "select TextSize from items where ItemID = '"
						+ maVatTu + "' LIMIT 1";
				c = dbHelper.getDb().rawQuery(query, null);
